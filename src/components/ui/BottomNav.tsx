...
import { Home, School, Timer, Bot, Users, Settings } from 'lucide-react';
...
export default function BottomNav() {
  ...
  const navItems = [
    ...
    { label: '학교', icon: School, path: '/school' },
    { label: '타이머', icon: Timer, path: '/timer' },
    { label: 'AI 퀴즈', icon: Bot, path: '/quiz' },
    { label: '스페이스', icon: Users, path: '/community' },
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] mt-1 font-semibold ${isActive ? 'text-blue-600 font-bold' : 'text-gray-400'}`}>
                {item.label}
              </span>
            </button>
          );
